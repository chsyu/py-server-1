...
def update_user(user_id: int, request: UpdateProfileRequestSchema, db: Session = Depends(get_db),
                current_user: UserRequestSchema =
                Depends(get_current_user_with_id)):
    return db_user.update(user_id=user_id, db=db, request=request)


@router.get('/all', response_model=List[UserResponseSchema])
def get_all_users(db: Session = Depends(get_db), current_user: UserRequestSchema =
Depends(get_current_user)):
    return db_user.get_all_users(db)


@router.get('/id/{user_id}', response_model=UserResponseWithProductsSchema)
def get_user_by_id(user_id: int, db: Session = Depends(get_db), current_user: UserRequestSchema =
Depends(get_current_user)):
    return db_user.get_user_by_id(user_id=user_id, db=db)


@router.get('/id_detail/{user_id}', response_model=UserDetailResponseSchema)
def get_user_detail_by_id(user_id: int, db: Session = Depends(get_db), current_user: UserRequestSchema =
Depends(get_current_user)):
...
def get_user_by_email(user_email: str, db: Session = Depends(get_db), current_user: UserRequestSchema =
Depends(get_current_user)):
...
def get_user_by_username(user_name: str, db: Session = Depends(get_db), current_user: UserRequestSchema =
Depends(get_current_user)):
